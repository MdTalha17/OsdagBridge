...
\hline"""
    ...
    if not pairs:
        ...
        cb_forces_rows.append(
            r"Between Girders & Diagonal & "
            + _ph("Section") + r" & " + _ph("$P_u$")
            + r" & C / T & " + _ph("$A_g$")
            + r" & " + _ph("$r$") + r" \\[6pt]\hline"
        )
    else:
        for pair in pairs:
            ...
            for member, label in [("diagonal", "Diagonal"),
                ...
                # Convert Area: cm² -> mm²
                area_str = f"{float(area_cm2) * 100:.1f}" if area_cm2 is not None else _ph("$A_g$")
                # Convert rv: cm -> mm
                rmin_str = f"{float(rv_cm) * 10:.1f}" if rv_cm is not None else _ph("$r_{min}$")
                cb_forces_rows.append(
...
class ReportDataBridge:
    ...
    def get_cb_efficiency(self, pair: str, member: str, force_type: str) -> str:
        ...
        return f"{float(val):.3f}" if val is not None else _ph("UR")
    ...
    def get_cb_gov_lc(self, pair: str, force_type: str) -> str:
        ...
        try:
            ...
            return _tex(val) if val else _ph("Load Case")
        except (KeyError, TypeError):
            return _ph("Load Case")
